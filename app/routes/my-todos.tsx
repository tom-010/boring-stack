...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const closedCount = todos.filter((t) => t.completed).length;

  return { todos, currentUserId: userId, openCount, closedCount };
}
        ...
        <div className="mb-8">
          ...
          <div className="flex gap-4 mt-4 text-sm">
            <span className="text-muted-foreground">
              Open: <span className="font-medium text-foreground">{loaderData.openCount}</span>
            </span>
            <span className="text-muted-foreground">
              Closed: <span className="font-medium text-foreground">{loaderData.closedCount}</span>
            </span>
          </div>
        </div>
